import pytest
import numpy as np
from FoldOptLib.datatypes.probability_distributions import Bounds, NormalDistribution, VonMisesFisherDistribution
...
def test_von_mises_fisher_distribution():
    von_mises_fisher_dist = VonMisesFisherDistribution(mu=[0,0,0], kappa=1, weight=0.5)
...
def test_von_mises_fisher_distribution_with_numpy():
    von_mises_fisher_dist = VonMisesFisherDistribution(mu=np.array([0, 10, 1]), kappa=1, weight=1)
