...
from FoldOptLib.objective_functions.objective_functions import (
    ObjectiveFunction,
    ObjectiveType,
)
...
def test_vonmises():
    # Test with valid inputs
    assert np.isclose(
        ObjectiveFunction.vonmises([0]), -vonmises(1e-10, 100).logpdf([0])
    )
...
def test_fourier_series():
    ...
    objective_fourier_series = ObjectiveFunction.fourier_series(
        rotation_angle, fold_frame_coordinate
    )
...
def test_angle_function():
    ...
    with pytest.raises(ValueError):
        ObjectiveFunction.angle_function(
            np.array([1, 0, 0]), np.array([[1, 0, 0], [0, 1, 0]])
        )
