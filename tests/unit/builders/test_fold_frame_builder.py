...
def test_fold_frame_builder_initialization(fold_frame_builder):
    ...
    assert np.array_equal(
        fold_frame_builder.xyz, constraints.data[["X", "Y", "Z"]].to_numpy()
    )
    assert fold_frame_builder.fold_frame == [None] * len(CoordinateType)
    assert (
        fold_frame_builder.constraints[CoordinateType.AXIAL_FOLIATION_FIELD] is not None
    )
...
def test_build_axial_surface_field(fold_frame_builder):
    ...
    assert (
        fold_frame_builder.fold_frame[CoordinateType.AXIAL_FOLIATION_FIELD] is not None
    )
